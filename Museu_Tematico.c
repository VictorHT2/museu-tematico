...
int verifica_cadastro()
{
    ...
    printf(" Já possui cadastro? (SIM = 1 / NÃO = 2) \n ");
    scanf("%d", &escolha);
    ...
    if(escolha == 1)
    {
		...
		while(fread(&r1, sizeof(REGISTRO),1,arquivo) == 1)
		{
			if(strcmp( email, r1.email) == 0)
			{
				...
				printf("\n As informações conferem? Se sim escreva 's' para continuar: ");
				scanf("%c", &certo);
...
void Usar_Bilhete(const char* email, int tema) {
    ...
    while (fread(&bilhete, sizeof(struct Bilhete), 1, arquivoB) == 1) {
        if (strcmp(bilhete.email, email) == 0 && bilhete.tema == tema && strcmp(bilhete.estado, "Disponivel") == 0) {
            ...
            novoBilhete.email[sizeof(novoBilhete.email) - 1] = '\0';  // Garantir terminação nula
            fwrite(&novoBilhete, sizeof(struct Bilhete), 1, tempArquivoB);
        } else {
            // Copiar bilhetes não modificados para o arquivo temporário
...
void cadastro()
{
	...
	{
		printf("\n Já tendo um cadastro, compre um ingresso e entre no museu.");
	}
	...
	if(verifica == 1)
	{
		...
		while(fread(&r1, sizeof(REGISTRO),1,arquivo) == 1)
		{
			if(strcmp( email, r1.email) == 0)
			{
				...
				if(r1.aniv.dia < 10 && r1.aniv.mes < 10)
				{
					printf("\n Seu aniversario é Aniversario: 0%d/0%d/%d", r1.aniv.dia, r1.aniv.mes, r1.aniv.ano);		
				}
				else if(r1.aniv.dia < 10)
				{
				printf("\n Seu aniversario é Aniversario: 0%d/%d/%d", r1.aniv.dia, r1.aniv.mes, r1.aniv.ano);
	
				}
				else if(r1.aniv.mes < 10)
				{
					printf("\n Seu aniversario é Aniversario: %d/0%d/%d", r1.aniv.dia, r1.aniv.mes, r1.aniv.ano);
				}
				else
				{
					printf("\n Seu aniversario é Aniversario: %d/%d/%d", r1.aniv.dia, r1.aniv.mes, r1.aniv.ano);
				}
				...
				printf("\n Data atual: %d/%d/%d", diaatual, mesatual, anoatual);
					...
					if(idade >= 60)
					{
						printf("\n Você paga meia na compra de um ingresso.\n \n ");						
					}
					...
					else
					{
						...
						printf("\n Você não paga meia na compra de um ingresso por idade.\n \n ");
						
						printf("\n Você possui alguma defifiencia ou alguma condição especial? (SIM = 1 / NÃO = 2)\n \n ");
						...
						{
							printf("\n Você paga inteira na compra de um ingresso.\n \n ");	
		...
		if(ingresso == 1)
		{
			...
			cabecalho();
			printf(" Você comprou um ingresso da Copa do Mundo \n ");			
		}
		else if(ingresso == 2)
		{
			continuar = 1;
			...
			cabecalho();
			printf(" Você comprou um ingresso das Olimpiadas \n ");
		}
		else if(ingresso == 3)
		{
			continuar = 1;
			...
			cabecalho();
			printf(" Você comprou um ingresso de Astrologia.");
		}
		else if(ingresso == 4)
		{
			continuar = 1;
			...
			cabecalho();
			printf(" Você comprou um ingresso da Copa do Mundo, Olimpiadas e de Astrologia.");

		}
	}	
	else
	{
		printf(" Vá para aba de cadastro !!");	
	}
...
void entrar()
{
	...
	{
		int escolha;
		printf(" Você gostaria de entrar na exposição da Copa do Mundo = 1 ou das Olimpiadas = 2 ou de Astrologia = 3 \n ");
    	scanf("%d", &escolha);	
    	...
    	if(escolha == 1 && bilhete1 != 0)
    	{
    		...
    		while(opc != 4)
    		{
    			...
    			printf(" Olhar Troféu da Copa do Mundo pressione 1.\n");
	    		...
	    		if(opc == 1)
	    		{
					...
					printf(" Diante de tantas novidades e curiosidades, uma\n coisa se mantém igual: o troféu. De acordo com\n a Conmebol, ele foi desenhado pelo escultor\n Silvio Gazzaniga, que trabalhava para a\n empresa Bertoni, em Milão, na Itália.\n\n O troféu mostra duas figuras de atletas segurando\n a Terra e, desde 1974, não mudou em nada: ele é\n feito de ouro 18 quilates, tem 36,8 cm de altura\n e pesa 6,1kg, segundo a FIFA.\n\n Apenas oito nações já levaram a taça, ou seja, um\n seleto grupo de dez por cento. Uruguai, Itália,\n Alemanha, Brasil, Inglaterra, Argentina, França e\n Espanha são os únicos campeões e, normalmente,\n favoritos a cada quatro anos.");    		
				...
				else if(opc== 2)
	    		{
					...
					printf("-------------------------------------\n");
 					...
 					printf(" Mascote La'eeb, que significa 'Jogador super\n habilidoso', é um personagem inspirado no\n lenço usado na cabeça das pessoas do mundo\n árabe.Trata-se de uma vestimenta tradicional\n popularmente conhecida por três nomes:Keffyeh,\n Shemagh e Ghutra.");	    		
	    			avaliacao(2);
				}
				else if(opc == 3)
	    		{
		...
		else if(escolha == 2 && bilhete2 != 0)
		{
    		...
    		while(opc != 4)
    		{
				...
				printf(" OLhar Estádio de Tokyo  pressione 3.\n");
				printf(" Para sair pressione 4.\n \n ");
				...
				else if(opc == 2)
	    		{
					...
					printf(" Por meio de uma campanha de coleta de\n metal, foram arrecadados 100% dos\n metais necessários para a produção das 5.000\n medalhas olímpicas de ouro, prata e bronze.\n Em dois anos de campanha a população contribuiu\n com aproximadamente 80 toneladas de aparelhos\n eletrônicos que resultaram em 32 kg de ouro,\n 3,500 kg de prata e 2,200 kg de bronze.");
				...
				else if(opc == 3)
	    		{
					...
					printf(" O Estádio Nacional do Japão foi palco\n dos jogos olímpicos de 1964 e foi\n remodelado para atender as Olimpíadas 2020.\n Na concepção do projeto, o arquiteto japonês\n Kengo Kuma utilizou madeira das 47 províncias\n que formam o Japão e adicionou no projeto\n paisagístico o plantio de aproximadamente\n 47.000 árvores.");
		...
		else if(escolha == 3 && bilhete3 != 0)
		{
    		...
    		while(opc != 4)
    		{
    			...
    			printf(" Olhar Pilares da criaçao pressione 1.\n");
				printf(" Olhar O Telescópio Espacial James Webb pressione 2.\n");
				...
				else if(opc == 2)
	    		{
					...
					printf(" O Telescópio Espacial James Webb, em inglês James\n Webb Space Telescope (JWST), é um telescópio espacial\n projetado pela Nasa em colaboração com a Agência Espacial\n Europeia (ESA) e a Agência Espacial Canadiana (CSA) com o\n intuito de substituir o Telescópio Espacial Hubble.\n\n 'Ele foi desenvolvido para responder aos questionamentos\n sobre a formação do universo, a existência de planetas\n habitáveis, a evolução de galáxias, estrelas, buracos\n negros e sistemas planetários e investigar com\n profundidade os exoplanetas.'");			
				...
				else if(opc == 3)
	    		{
	    			...
	    			printf(" James Webb descobriu um buraco negro supermassivo\n na galáxia CEERS 1019 que está a 13,2 bilhões de\n anos-luz de distância do nosso planeta, que tem uma\n massa de 10 milhões de vezes a do sol no centro de\n uma galáxia bebê formada 570 milhões de anos após\n o surgimento do universo.");			
    	...
    	{
    		cabecalho();
			printf(" Escolheu a Copa do Mundo mas não tem ingresso, vá na bilheteria !!\n");		
		}
		else if(escolha == 2 && bilhete2 == 0)
		{
			cabecalho();
			printf(" Escolheu as Olimpiadas mas não tem ingresso, vá na bilheteria !!\n");		
		}
				else if(escolha == 3 && bilhete3 == 0)
		{
			cabecalho();
    		printf(" Escolheu a Astrologia e não tem ingresso !!\n");
	...
	{
		printf(" Você não possui bilhetes de entrada, vá na bilheteria !!");	
	}
